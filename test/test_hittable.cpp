...
namespace UnitTest {
	...
	void testHitTScalesAndTRange(const Hittable& hittable, const Ray& ray, const Hittable::HitRecord& expectHit, float expectT, float safeTDist, float tolerance) {
		testHitTRange(hittable, ray, expectHit, expectT, safeTDist, tolerance);
		testHitTRange(hittable, Ray(ray.origin(), ray.direction() * 0.1f), expectHit, expectT / 0.1f, safeTDist / 0.1f, tolerance);
		testHitTRange(hittable, Ray(ray.origin(), ray.direction() * 0.5f), expectHit, expectT / 0.5f, safeTDist / 0.5f, tolerance);
		testHitTRange(hittable, Ray(ray.origin(), ray.direction() * 3.f), expectHit, expectT / 3.f, safeTDist / 3.f, tolerance);
		testHitTRange(hittable, Ray(ray.origin(), ray.direction() * 10.f), expectHit, expectT / 10.f, safeTDist / 10.f, tolerance);
	}
}